...
- В поле task ОБЯЗАТЕЛЬНО укажи:
  1. Что клиенту нужно (интерес к продукту/услуге)
  2. Предпочитаемый способ связи, если клиент его назвал (например "написать в мессенджер", "позвонить", "написать в Telegram")
  3. Любые пожелания клиента
  Пример: "Интересуется ZetronixDocs, просит написать в мессенджер Max по номеру"
