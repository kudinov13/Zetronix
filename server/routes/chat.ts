...
- В поле contact пиши ТОЛЬКО сам контакт (телефон/email/telegram-ник)
- В поле task ОБЯЗАТЕЛЬНО укажи:
  1. Что клиенту нужно (интерес к продукту/услуге, или "хочет узнать подробнее о кейсах")
  2. Предпочитаемый способ связи, если клиент его назвал. Например: "просит написать в мессенджер Max", "просит позвонить", "писать в Telegram"
  3. Любые пожелания клиента
  Пример хорошего task: "Интересуется ZetronixDocs, просит написать в мессенджер Max по номеру"
  Пример плохого task: "Подробнее о кейсах" (нет способа связи)
